...
px4adrc::ControlOutput run_controller_steps(
  px4adrc::AdrcController & controller,
  const px4adrc::VehicleState & state,
  const px4adrc::TrajectoryReference & ref,
  int steps,
  double dt)
{
...
TEST(AdrcController, RateNlsefGeneratesTorqueFromRateError)
{
  px4adrc::ControllerParams params{};
  params.max_total_thrust_n = 60.0;
  for (auto & gains : params.position_nlsef_gains) {
  ...
  for (auto & gains : params.rate_nlsef_gains) {
    gains.k1 = 0.0;
    gains.k2 = 0.0;
  }
  params.rate_nlsef_gains[0].k1 = 0.8;
  params.rate_nlsef_gains[0].alpha1 = 1.0;
  px4adrc::AdrcController controller(params);
