...
void update_eso(double measurement, double control_input, ExtendedStateObserver &eso, double state_derivative_offset) {
  const double error = eso.z1 - measurement;
  eso.z1 += eso.h * (eso.z2 - state_derivative_offset - eso.beta1 * error);
...
double compute_nlsef(double e1, double e2, const NlsefGains &gains) {
  return gains.k1 * fal(e1, gains.alpha1, gains.delta) + gains.k2 * fal(e2, gains.alpha2, gains.delta);
}
