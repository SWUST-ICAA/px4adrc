...
std::array<double, 3>
vector_param_or_default(rclcpp::Node &node, const std::string &name,
...
void assign_axis_gains(std::array<NlsefGains, 3> &target,
...
void assign_td_axis_gains(std::array<TrackingDifferentiatorGains, 3> &target,
...
void assign_eso_axis_gains(std::array<EsoGains, 3> &target,
...
bool reference_message_has_valid_position(
    const px4adrc::msg::FlatTrajectoryReference &msg) {
  return std::isfinite(msg.position_ned.x) &&
         std::isfinite(msg.position_ned.y) && std::isfinite(msg.position_ned.z);
}

TrajectoryReference
trajectory_reference_from_msg(const px4adrc::msg::FlatTrajectoryReference &msg,
  ...
  ref.position_ned = Eigen::Vector3d(msg.position_ned.x, msg.position_ned.y,
                                     msg.position_ned.z);
  ref.velocity_ned = Eigen::Vector3d(sanitize_scalar(msg.velocity_ned.x),
                                     sanitize_scalar(msg.velocity_ned.y),
                                     sanitize_scalar(msg.velocity_ned.z));
  ref.acceleration_ned =
      Eigen::Vector3d(sanitize_scalar(msg.acceleration_ned.x),
                      sanitize_scalar(msg.acceleration_ned.y),
                      sanitize_scalar(msg.acceleration_ned.z));
  ref.body_rates_frd = Eigen::Vector3d(sanitize_scalar(msg.body_rates_frd.x),
                                       sanitize_scalar(msg.body_rates_frd.y),
                                       sanitize_scalar(msg.body_rates_frd.z));
  ref.body_torque_frd = Eigen::Vector3d(sanitize_scalar(msg.body_torque_frd.x),
...
MissionState next_mission_state(MissionState current, bool offboard_enabled,
                                bool armed, bool takeoff_complete,
                                bool reference_fresh) {
  switch (current) {
  case MissionState::WAIT_FOR_STATE:
    return MissionState::REQUEST_OFFBOARD;
  case MissionState::REQUEST_OFFBOARD:
    return offboard_enabled ? MissionState::WAIT_FOR_MANUAL_ARM
...
bool should_publish_start_tracking_signal(MissionState state,
...
void AdrcNode::declare_parameters() {
  ...
  this->declare_parameter<std::string>("topics.start_tracking",
                                       topic_start_tracking_);
  this->declare_parameter<double>("control_rate_hz", control_rate_hz_);
  this->declare_parameter<double>("mission.takeoff_target_z",
                                  takeoff_target_z_ned_);
  this->declare_parameter<double>("mission.takeoff_z_threshold",
                                  takeoff_z_threshold_m_);
  this->declare_parameter<double>("mission.takeoff_hold_time_s",
                                  takeoff_hold_time_s_);
  this->declare_parameter<double>("mission.reference_timeout_s",
                                  reference_timeout_s_);
  this->declare_parameter<double>("position_adrc.max_tilt_deg", 35.0);
  this->declare_parameter<std::vector<double>>(
      "position_adrc.td_r",
      {defaults.position_td_gains[0].r, defaults.position_td_gains[1].r,
       defaults.position_td_gains[2].r});
  this->declare_parameter<std::vector<double>>(
      "position_adrc.eso_beta1", {defaults.position_eso_gains[0].beta1,
                                  defaults.position_eso_gains[1].beta1,
                                  defaults.position_eso_gains[2].beta1});
  this->declare_parameter<std::vector<double>>(
      "position_adrc.eso_beta2", {defaults.position_eso_gains[0].beta2,
                                  defaults.position_eso_gains[1].beta2,
                                  defaults.position_eso_gains[2].beta2});
  this->declare_parameter<std::vector<double>>(
      "position_adrc.eso_beta3", {defaults.position_eso_gains[0].beta3,
                                  defaults.position_eso_gains[1].beta3,
  ...
  this->declare_parameter<std::vector<double>>(
      "position_adrc.nlsef_alpha1", {defaults.position_nlsef_gains[0].alpha1,
                                     defaults.position_nlsef_gains[1].alpha1,
                                     defaults.position_nlsef_gains[2].alpha1});
  this->declare_parameter<std::vector<double>>(
      "position_adrc.nlsef_alpha2", {defaults.position_nlsef_gains[0].alpha2,
                                     defaults.position_nlsef_gains[1].alpha2,
                                     defaults.position_nlsef_gains[2].alpha2});
  this->declare_parameter<std::vector<double>>(
      "position_adrc.nlsef_delta", {defaults.position_nlsef_gains[0].delta,
                                    defaults.position_nlsef_gains[1].delta,
  ...
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_eso_beta1",
      {defaults.attitude_eso_gains[0].beta1,
       defaults.attitude_eso_gains[1].beta1,
       defaults.attitude_eso_gains[2].beta1});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_eso_beta2",
      {defaults.attitude_eso_gains[0].beta2,
       defaults.attitude_eso_gains[1].beta2,
       defaults.attitude_eso_gains[2].beta2});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_eso_beta3",
      {defaults.attitude_eso_gains[0].beta3,
       defaults.attitude_eso_gains[1].beta3,
       defaults.attitude_eso_gains[2].beta3});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_eso_b0",
      {defaults.attitude_eso_gains[0].b0, defaults.attitude_eso_gains[1].b0,
       defaults.attitude_eso_gains[2].b0});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_nlsef_k1",
      {defaults.attitude_nlsef_gains[0].k1, defaults.attitude_nlsef_gains[1].k1,
       defaults.attitude_nlsef_gains[2].k1});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_nlsef_k2",
      {defaults.attitude_nlsef_gains[0].k2, defaults.attitude_nlsef_gains[1].k2,
       defaults.attitude_nlsef_gains[2].k2});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_nlsef_alpha1",
      {defaults.attitude_nlsef_gains[0].alpha1,
       defaults.attitude_nlsef_gains[1].alpha1,
       defaults.attitude_nlsef_gains[2].alpha1});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_nlsef_alpha2",
      {defaults.attitude_nlsef_gains[0].alpha2,
       defaults.attitude_nlsef_gains[1].alpha2,
       defaults.attitude_nlsef_gains[2].alpha2});
  this->declare_parameter<std::vector<double>>(
      "attitude_adrc.attitude_nlsef_delta",
      {defaults.attitude_nlsef_gains[0].delta,
       defaults.attitude_nlsef_gains[1].delta,
       defaults.attitude_nlsef_gains[2].delta});
  this->declare_parameter<double>("filters.velocity_diff_cutoff_hz",
                                  velocity_diff_cutoff_hz_);
  this->declare_parameter<double>("filters.body_rate_diff_cutoff_hz",
                                  body_rate_diff_cutoff_hz_);
  this->declare_parameter<double>("vehicle.mass_kg", 1.0);
  this->declare_parameter<double>("vehicle.gravity", 9.81);
  this->declare_parameter<std::vector<double>>("vehicle.inertia_diag",
                                               {0.01, 0.01, 0.02});
...
void AdrcNode::load_parameters() {
  ...
  params.yaw_moment_coeff =
      this->get_parameter("vehicle.yaw_moment_coeff").as_double();
  params.max_total_thrust_n =
      this->get_parameter("vehicle.max_total_thrust_n").as_double();
  params.thrust_model_factor =
      this->get_parameter("motors.thrust_model_factor").as_double();
  params.max_tilt_rad =
      this->get_parameter("position_adrc.max_tilt_deg").as_double() * M_PI /
      180.0;

  const auto inertia_diag =
      this->get_parameter("vehicle.inertia_diag").as_double_array();
  if (inertia_diag.size() == 3) {
    params.inertia_diag =
        Eigen::Vector3d(inertia_diag[0], inertia_diag[1], inertia_diag[2]);
  }

  assign_td_axis_gains(
      params.position_td_gains,
      vector_param_or_default(*this, "position_adrc.td_r",
                              {params.position_td_gains[0].r,
                               params.position_td_gains[1].r,
...
void AdrcNode::create_ros_interfaces() {
  ...
  odom_sub_ = this->create_subscription<px4_msgs::msg::VehicleOdometry>(
      "/fmu/out/vehicle_odometry", qos_px4_out,
  ...
  status_sub_ = this->create_subscription<px4_msgs::msg::VehicleStatus>(
      ...
      std::bind(&AdrcNode::vehicle_status_callback, this,
                std::placeholders::_1));
  reference_sub_ =
      this->create_subscription<px4adrc::msg::FlatTrajectoryReference>(
...
void AdrcNode::odom_callback(
  ...
  if (msg->pose_frame != px4_msgs::msg::VehicleOdometry::POSE_FRAME_NED) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                         "vehicle_odometry pose frame is not NED, controller "
                         "still interprets data as NED");
  }

  const uint64_t timestamp_us = msg->timestamp;
  double dt = 0.0;
  if (last_odom_us_ != 0 && timestamp_us > last_odom_us_) {
    dt = std::clamp(static_cast<double>(timestamp_us - last_odom_us_) * 1e-6,
                    1e-4, 0.05);
  }
  ...
  if (dt > 0.0) {
    const Eigen::Vector3d raw_acceleration_ned =
        (state_.velocity_ned - last_velocity_ned_) / dt;
    const Eigen::Vector3d raw_body_accel_frd =
        (state_.body_rates_frd - last_body_rates_frd_) / dt;
    state_.acceleration_ned =
        low_pass_vec3(state_.acceleration_ned, raw_acceleration_ned,
                      velocity_diff_cutoff_hz_, dt);
    state_.body_accel_frd =
        low_pass_vec3(state_.body_accel_frd, raw_body_accel_frd,
                      body_rate_diff_cutoff_hz_, dt);
  }
...
void AdrcNode::vehicle_status_callback(
...
void AdrcNode::reference_callback(
    const px4adrc::msg::FlatTrajectoryReference::SharedPtr msg) {
  if (!msg) {
    return;
  }
...
bool AdrcNode::has_fresh_reference(uint64_t now_us) const {
  ...
  return static_cast<double>(now_us - last_reference_receive_us_) * 1e-6 <=
         reference_timeout_s_;
}
...
void AdrcNode::publish_vehicle_command(uint16_t command, float param1,
                                       float param2, float param3) {
  px4_msgs::msg::VehicleCommand msg{};
  msg.timestamp = state_.timestamp_us != 0 ? state_.timestamp_us
                                           : now_micros(*this->get_clock());
  msg.param1 = param1;
  msg.param2 = param2;
  msg.param3 = param3;
  msg.command = command;
...
void AdrcNode::publish_actuator_motors(
    uint64_t timestamp_us, uint64_t timestamp_sample_us,
...
void AdrcNode::publish_vehicle_thrust_setpoint(
  ...
  msg.xyz[1] = 0.0F;
  msg.xyz[2] =
      static_cast<float>(-collective_thrust_normalized_from_motor_thrusts(
          motor_thrusts_n, controller_.params()));
...
void AdrcNode::control_loop() {
  if (exit_requested_) {
    RCLCPP_ERROR(
        this->get_logger(),
        "Offboard mode exited after engagement, shutting down controller");
    rclcpp::shutdown();
    return;
  }
  ...
  const uint64_t now_us = state_.timestamp_us != 0
                              ? state_.timestamp_us
                              : now_micros(*this->get_clock());
  ...
  if (mission_state_ == MissionState::TAKEOFF ||
    ...
    takeoff_complete =
        std::abs(state_.position_ned.z() - takeoff_target_z_ned_) <=
        takeoff_z_threshold_m_;
  }

  const bool reference_fresh = has_fresh_reference(now_us);
  const MissionState next_state =
      next_mission_state(mission_state_, is_offboard_, is_armed_,
                         takeoff_complete, reference_fresh);
  ...
  const bool hold_time_elapsed =
      mission_state_ == MissionState::HOLD &&
      static_cast<double>(now_us - hold_enter_us_) * 1e-6 >=
          takeoff_hold_time_s_;
  if (should_publish_start_tracking_signal(mission_state_, hold_time_elapsed,
                                           start_tracking_sent_)) {
    publish_start_tracking_signal(true);
    start_tracking_sent_ = true;
  }

  if (mission_state_ == MissionState::HOLD && hold_time_elapsed &&
      reference_fresh) {
    mission_state_ = MissionState::TRACKING;
  }
  ...
  publish_vehicle_thrust_setpoint(now_us, state_.timestamp_us,
                                  output.motor_thrusts_n);
}
