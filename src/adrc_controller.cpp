...
void apply_td_gains(std::array<TrackingDifferentiator, 3> &td_states,
...
void apply_eso_gains(std::array<ExtendedStateObserver, 3> &eso_states,
...
Eigen::Vector3d
update_position_channel(const VehicleState &state,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    const double e2 = pos_td[i].x2 + sanitize_scalar(ref.velocity_ned[i]) -
                      state.velocity_ned[i];
    accel_cmd[i] +=
        compute_nlsef(e1, e2, params.position_nlsef_gains[i]) - pos_eso[i].z3;
...
Eigen::Quaterniond current_to_desired_body_quaternion(
    const Eigen::Quaterniond &current_q_body_to_ned,
    const Eigen::Quaterniond &desired_q_body_to_ned) {
  Eigen::Quaterniond q_current_to_desired =
      desired_q_body_to_ned.conjugate() *
...
Eigen::Vector3d
update_attitude_channel(const VehicleState &state,
                        const Eigen::Quaterniond &desired_q_body_to_ned,
  ...
  const Eigen::Quaterniond q_current_to_desired =
      current_to_desired_body_quaternion(state.q_body_to_ned,
                                         desired_q_body_to_ned);
  const Eigen::Vector3d attitude_error_desired_frd =
      2.0 * q_current_to_desired.vec();
  const Eigen::Vector3d current_body_rates_desired_frd =
      q_current_to_desired * state.body_rates_frd;

  Eigen::Vector3d torque_cmd_desired_frd =
      sanitize_vec3(torque_feedforward_frd);
...
ControlOutput AdrcController::update(const VehicleState &state,
                                     const TrajectoryReference &ref,
                                     double dt) {
  const double clamped_dt = std::clamp(dt, 1e-4, 0.05);
  const Eigen::Vector3d accel_cmd_ned = update_position_channel(
      state, ref, pos_td_, pos_eso_, last_position_accel_cmd_ned_, params_,
      clamped_dt);
  ...
  const double total_thrust_n = clamp_scalar(
      params_.mass_kg * thrust_vector_norm, 0.0, params_.max_total_thrust_n);
  ...
  const Eigen::Quaterniond desired_q_body_to_ned =
      quaternion_from_thrust_direction_yaw_ned(thrust_direction_ned,
                                               sanitize_scalar(ref.yaw));
  const Eigen::Vector3d torque_cmd = update_attitude_channel(
      state, desired_q_body_to_ned, sanitize_vec3(ref.body_rates_frd),
      sanitize_vec3(ref.body_torque_frd), attitude_td_, attitude_eso_,
  ...
  output.motor_throttles =
      motor_thrusts_to_throttles(output.motor_thrusts_n, params_);
  return output;
}
