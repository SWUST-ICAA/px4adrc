...
inline double clamp_scalar(double value, double lower, double upper)
{
  return std::min(std::max(value, lower), upper);
}

inline double sanitize_scalar(double value)
{
  return std::isfinite(value) ? value : 0.0;
}

inline Eigen::Vector3d sanitize_vec3(const Eigen::Vector3d & value)
{
  return value.allFinite() ? value : Eigen::Vector3d::Zero();
}

inline double low_pass_alpha(double cutoff_hz, double dt)
{
  ...
  return clamp_scalar(dt / (rc + dt), 0.0, 1.0);
}

inline Eigen::Vector3d low_pass_vec3(
  const Eigen::Vector3d & previous,
  const Eigen::Vector3d & sample,
  double cutoff_hz,
  double dt)
{
  const double alpha = low_pass_alpha(cutoff_hz, dt);
...
inline Eigen::Quaterniond align_quaternion_sign(
  const Eigen::Quaterniond & quaternion,
  const Eigen::Quaterniond & anchor)
{
...
inline Eigen::Quaterniond quaternion_from_thrust_direction_yaw_ned(
  const Eigen::Vector3d & thrust_direction_ned,
  double yaw)
{
  Eigen::Vector3d z_body_ned = sanitize_vec3(thrust_direction_ned);
...
inline std::array<double, kMotorCount> clamp_motor_thrusts(
  const std::array<double, kMotorCount> & motor_thrusts_n,
...
inline std::array<double, kMotorCount> motor_thrusts_to_throttles(
  const std::array<double, kMotorCount> & motor_thrusts_n,
  const ControllerParams & params)
{
...
inline double collective_thrust_normalized_from_motor_thrusts(
  const std::array<double, kMotorCount> & motor_thrusts_n,
  const ControllerParams & params)
{
  const auto thrusts = clamp_motor_thrusts(motor_thrusts_n, params);
...
inline std::array<double, kMotorCount> quad_x_allocate(
  ...
  const ControllerParams & params)
{
  ...
  thrusts[0] = 0.25 * total_thrust_n
    - 0.25 * torque_frd.x() / arm_xy
    + 0.25 * torque_frd.y() / arm_xy
    + 0.25 * torque_frd.z() / yaw_coeff;
  thrusts[1] = 0.25 * total_thrust_n
    + 0.25 * torque_frd.x() / arm_xy
    - 0.25 * torque_frd.y() / arm_xy
    + 0.25 * torque_frd.z() / yaw_coeff;
  thrusts[2] = 0.25 * total_thrust_n
    + 0.25 * torque_frd.x() / arm_xy
    + 0.25 * torque_frd.y() / arm_xy
    - 0.25 * torque_frd.z() / yaw_coeff;
  thrusts[3] = 0.25 * total_thrust_n
    - 0.25 * torque_frd.x() / arm_xy
    - 0.25 * torque_frd.y() / arm_xy
    - 0.25 * torque_frd.z() / yaw_coeff;
